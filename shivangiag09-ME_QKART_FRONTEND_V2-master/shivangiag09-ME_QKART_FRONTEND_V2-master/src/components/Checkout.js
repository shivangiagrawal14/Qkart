import { CreditCard, Delete, FamilyRestroomRounded } from "@mui/icons-material";
import {
  Button,
  Divider,
  Grid,
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    ...
    try {
      ...
      const response=await axios.post(`${config.endpoint}/user/addresses`, data, {
        headers: {
          'Authorization': `Bearer ${token}`
        },
      })
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      const response=await axios.post(`${config.endpoint}/cart/checkout`, data, {
         headers: {
           'Authorization': `Bearer ${token}`
         },
       })
              ...
              {addresses.all.length===0 ?
               ...
               :
                 (addresses.all.map((item)=>{
                  return (
                  <Box  key = {item.address} className={item.address===addresses.selected.address ? "address-item selected": "address-item not-selected"}>
                    <Box onClick={()=>{
                      setAddresses({...addresses,selected:{address: item.address, id: item._id}})
                    }}>{item.address}</Box>
